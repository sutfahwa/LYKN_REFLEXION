// POST /admin-edit-announcement — แก้ไขประกาศที่เคยส่งไปแล้ว
...
Deno.serve(async (req) => {
  ...
  if (!id || !title || !bodyText || (isClickable && !detail)) {
    return jsonResponse({ error: 'MISSING_FIELDS' }, 400);
  }

  const admin = supabaseAdmin();
  const result = await admin.rpc('admin_edit_announcement', {
    p_id: id, p_title: title, p_body: bodyText, p_is_clickable: isClickable, p_detail: detail, p_admin_id: user.id,
  });
